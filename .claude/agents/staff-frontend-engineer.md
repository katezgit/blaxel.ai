...
| **Empty / zero-record state for a list, table, or surface** | [foundations/empty-state.md](../../docs/design/foundations/empty-state.md) — rendering decision: when the chrome (header, filters, identity) persists around the empty content vs when the whole surface dissolves into a single message |
| **Page header / title block spacing and rhythm** | [foundations/header-rhythm.md](../../docs/design/foundations/header-rhythm.md) — vertical spacing between title, subtitle, breadcrumbs, action row, and first content block |
| **Toast / banner / inline notification wiring** | [patterns/notifications.md](../../docs/design/patterns/notifications.md) — channel binding (toast vs alert vs inline) per event class; orchestrator vs consumer ownership |
| **Pattern-level decision (cross-component composition)** | [patterns/index.md](../../docs/design/patterns/index.md) — pattern entry point; consult before pattern-matching against external precedent |
| **Stack / framework lookup (versions, primitives, conventions)** | [tech-stack.md](../../docs/tech-stack.md) — project stack (Next.js / React 19 / Tailwind v4 / TanStack Query / etc.) |
...
| **Any empty / error / loading state surface** | [empty-and-error-states.md](../../docs/design/guidelines/empty-and-error-states.md) — decision tree (ErrorState / Alert / Toast / EmptyState / inline), copy structure, required states |
| **Building a list / index page (any table view)** | [tables.md](../../docs/design/guidelines/tables.md) — row archetypes (catalog / navigation / selection), column order, action placement, when to add row selection |
| **Building the filter / search row above a table** | [toolbar.md](../../docs/design/guidelines/toolbar.md) — split-anchor rule (search left, segments / filters / refinements right), locked order |
| **Building a multi-step form (wizard / stepper)** | [stepper-actions.md](../../docs/design/guidelines/stepper-actions.md) — footer button group (Prev / Cancel / forward), split-anchor layout, container constraints                  |
| **Implementing a resource-detail 404 (`/<type>/<id>`)** | [resource-not-found.md](../../docs/design/guidelines/resource-not-found.md) — 404/403 collapse, nav-chrome preservation, wrong-workspace recovery affordance                  |
| **Diverging from a guideline (or wondering why we don't follow the industry default)** | [house-rules.md](../../docs/design/guidelines/house-rules.md) — deliberate forks from industry consensus (e.g. no drawer-based wizards); consult before pattern-matching against external prior art |
