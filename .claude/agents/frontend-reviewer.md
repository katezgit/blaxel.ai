...
- **App folder structure (FAIL).** Read [`docs/conventions/app-conventions.folder-structure.md`](../../docs/conventions/app-conventions.folder-structure.md). Single-route components MUST live in `app/<route>/_components/`, not in `apps/<app>/src/components/`. Shared (≥2 routes) → `src/components/`. App-shell / providers / route-agnostic UI → `src/components/`. FAIL PRs that put page-only components in shared folders OR put them loose under the route folder (must be inside `_components/`).
...
- **Typography:** read [`docs/design/foundations/typography.md`](../../docs/design/foundations/typography.md) and FAIL on the listed off-scale triggers. The canonical scale is three composite utilities only — `typography-display` (24/600/-0.02em), `typography-subtitle` (16/600/-0.01em), `typography-body` (14/400/0) — declared once at a parent and inherited. Common FAIL triggers: Tailwind size defaults (`text-xs`/`text-sm`/`text-base`/`text-lg`/`text-xl`/`text-2xl` — `primitive.css` wipes these via `--text-*: initial`, so they are no-ops), independent `leading-*` or `tracking-*` applied on top of a composite (the composite already sets both — overriding fragments the system), weights other than 400 / 600 at consumer call sites (`font-medium` 500, `font-bold` 700), redundant `typography-body` on a descendant when the ancestor already declares it. For the parent-declares-once / leaf-overrides-only inheritance pattern with worked examples, read [`docs/conventions/typography-utility-usage.md`](../../docs/conventions/typography-utility-usage.md).
- **Container choice:** read [`docs/design/guidelines/container-choice.md`](../../docs/design/guidelines/container-choice.md) and FAIL on the listed anti-patterns. Quote the rubric verbatim. Common FAIL triggers: dialog-on-drawer stacking, high-stakes resource creation in a drawer, confirm-only action on a page, >3 fields in a dialog, multi-step wizard in a drawer, drawer where context is irrelevant, card-wrap inside dialog or drawer.
- **Dismiss confirmation:** read [`docs/design/guidelines/form-actions.md`](../../docs/design/guidelines/form-actions.md) §7. Two-condition gate: dismiss confirmation fires ONLY when BOTH form is dirty AND data entry is high-stakes. FAIL if the confirm fires on a clean form (trains reflexive dismissal) OR is omitted on a dirty high-stakes form. Low-stakes dirty forms (inline rename, single toggle, single numeric field) dismiss silently.
- **App shell (sidebar / topbar / content widths):** read [`docs/design/guidelines/app-shell-layout.md`](../../docs/design/guidelines/app-shell-layout.md) and FAIL on shell anti-patterns — wrong shell archetype for the destination count, content-width violations, duplicated nav scaffolding.
- **Empty / error / loading states:** read [`docs/design/guidelines/empty-and-error-states.md`](../../docs/design/guidelines/empty-and-error-states.md) and FAIL on wrong component for the situation (ErrorState vs Alert vs Toast vs EmptyState vs inline field error), missing required states, or copy that violates the structure rules.
- **Resource-not-found (`/<type>/<id>` 404):** read [`docs/design/guidelines/resource-not-found.md`](../../docs/design/guidelines/resource-not-found.md) and FAIL on misapplied 404/403 collapse, missing nav-chrome preservation for authenticated-in-context 404s, or surfacing deletion details that should stay collapsed.
- **Tables (any list / index view):** read [`docs/design/guidelines/tables.md`](../../docs/design/guidelines/tables.md) and FAIL on wrong row archetype (catalog / navigation / selection), action placement that conflicts with the archetype, mis-ordered columns, or row selection added where the archetype forbids it.
- **Table anatomy / tokens / states:** read [`docs/design/components/table/spec.md`](../../docs/design/components/table/spec.md) and FAIL on missing required states (default / hover / selected / loading / empty), wrong token bindings, or anatomy that diverges from the spec.
- **Table cell typography:** read [`docs/design/components/table/typography.md`](../../docs/design/components/table/typography.md) and FAIL on wrong typography tier per cell role (header / numeric / monospace / muted).
- **Empty / zero-record states:** read [`docs/design/foundations/empty-state.md`](../../docs/design/foundations/empty-state.md) and FAIL on wrong rendering decision — chrome (header, filters, identity) must persist around the empty content for surfaces with stable identity; only fully-undefined surfaces dissolve into a single message. Stripping nav / header chrome on an empty list = FAIL.
- **Page header rhythm:** read [`docs/design/foundations/header-rhythm.md`](../../docs/design/foundations/header-rhythm.md) and FAIL on off-spec spacing between title, subtitle, breadcrumbs, action row, and first content block.
- **Toolbar (filter row above a table):** read [`docs/design/guidelines/toolbar.md`](../../docs/design/guidelines/toolbar.md) and FAIL on split-anchor violations (search must anchor LEFT; segments / filters / refinements anchor RIGHT in the locked internal order).
- **Stepper / multi-step form:** read [`docs/design/guidelines/stepper-actions.md`](../../docs/design/guidelines/stepper-actions.md) and FAIL on wrong container (no drawer-based wizards — see `house-rules.md` §1), incorrect Prev / Cancel / forward button group, or non-split-anchor footer layout.
- **House rules (deliberate divergence from industry default):** read [`docs/design/guidelines/house-rules.md`](../../docs/design/guidelines/house-rules.md) when an external-pattern argument is invoked. Industry consensus does not override our house rules — FAIL implementations that pattern-match external prior art instead of the listed house rule.
