...
Tanstack Query v5 key conventions for cache management and invalidation in the Blaxel portal. The shape is tenancy-first because workspaces depend on user and resources depend on workspace — switching user or workspace must drop every dependent cache entry in one prefix invalidation.
