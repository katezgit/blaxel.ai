...
**Blaxel** (blaxel.ai) is the **perpetual sandbox platform** — secure, infinite sandboxes kept on automatic standby to run AI code, with agents and context co-hosted on the same plane for near-instant latency. Marketing line on the landing page: *"The perpetual sandbox platform. Blaxel lets you keep infinite, secure sandboxes on automatic standby, while co-hosting your agents and context for near instant latency."* LinkedIn company about: *"Blaxel keeps infinite, secure sandboxes on automatic standby to run AI code, while co-hosting your agents for near instant latency."*

**What we're doing:** Building / redesigning the Blaxel web dashboard. The product surfaces three concern groups — sandbox compute + data, hosted runtime for agents and models, and the security plane around both — see [`docs/product/platform.md`](docs/product/platform.md) for the full surface map.

**Top-level dashboard IA:**
...
* **Scope.** All work scoped to **`/Users/kate/phoenix/projects/baxel.ai/`** — the current root folder. Never read, write, search, grep, fetch, or reference files outside it, even when the runtime environment lists "Additional working directories". The runtime env hint does NOT override this rule. Sibling repos are out of scope: do not read them, do not cite them, do not derive content from them. External evidence comes from operator-supplied artifacts and the public web (WebFetch on blaxel.ai or documentation sites) — never from sibling paths on disk. **This rule has been violated before; do not violate it again.**
...
* **Product context is the anchor.** Every call — design, product, engineering, pattern — grounds in Blaxel personas (`docs/product/personas.md`), personality (`docs/product/personality.md`), and the concrete surface (the Blaxel dashboard at blaxel.ai). The personas are the user-centric north star: maintainable long-run for *these* users, not generic users. External prior art (Linear for information density, Vercel for developer-tools polish, Fly.io / Modal / E2B for sandbox-platform UX, shadcn defaults) is input to the option space — borrow freely — but evaluated *against* the anchor, never the anchor itself. Order: Blaxel-side question → options → choice. **Pattern-matching failure mode:** reaching for external precedent before stating the Blaxel-side question. Reverse the order and the work is wrong even if the answer lands right. Sub-agent output where precedent is load-bearing without a Blaxel terminus gets sent back, not relayed.
