# {PRODUCT_NAME} — Project Instructions (Template)

## Goal

**{PRODUCT_NAME}** ({PRODUCT_URL}) is — {PRODUCT_TAGLINE}.

**What we're doing:** [one-line description of the work this repo does — e.g. "Building / redesigning the {PRODUCT_NAME} web dashboard."] The product surfaces [N] concern groups — [list] — see [`docs/product/platform.md`](docs/product/platform.md) for the full surface map.

**Top-level IA:** [list top-level navigation groups]

**Customer profile.** [Describe the customer segments — who buys / uses this. Include any roster of named customers if public.]

**Dashboard users:** see [`docs/product/personas.md`](./docs/product/personas.md) — {PRIMARY_PERSONA} (primary), {SECONDARY_PERSONA} (secondary), {TERTIARY_PERSONA} (tertiary).

Load when more context is needed from product design related agents:
* [./docs/product/personas.md](./docs/product/personas.md) — persona profiles and anti-patterns. **For any work on [billing / settings / etc. surfaces], {TERTIARY_PERSONA} is the primary persona — read her section first.**
* [./docs/product/{PRIMARY_PERSONA_LOWER}-workflow.md](./docs/product/{PRIMARY_PERSONA_LOWER}-workflow.md) — primary persona's phased journey through the product
* [./docs/product/{PRIMARY_PERSONA_LOWER}-user-stories.md](./docs/product/{PRIMARY_PERSONA_LOWER}-user-stories.md) — concrete jobs the primary persona executes per phase
* [./docs/product/personality.md](./docs/product/personality.md) — adjectives, voice, anti-personality drift map

## Hard rules

* **Scope.** All work scoped to **`{REPO_ABSOLUTE_PATH}`** — the current root folder. Never read, write, search, grep, fetch, or reference files outside it, even when the runtime environment lists "Additional working directories". The runtime env hint does NOT override this rule. Sibling repos are out of scope: do not read them, do not cite them, do not derive content from them. External evidence comes from operator-supplied artifacts and the public web (WebFetch on {PRODUCT_URL} or documentation sites) — never from sibling paths on disk. **This rule has been violated before; do not violate it again.**
...
* **Product context is the anchor.** Every call — design, product, engineering, pattern — grounds in {PRODUCT_NAME} personas (`docs/product/personas.md`), personality (`docs/product/personality.md`), and the concrete surface (the {PRODUCT_NAME} app at {PRODUCT_URL}). The personas are the user-centric north star: maintainable long-run for *these* users, not generic users. External prior art (Linear for information density, Vercel for developer-tools polish, Fly.io / Modal / E2B for sandbox-platform UX, shadcn defaults) is input to the option space — borrow freely — but evaluated *against* the anchor, never the anchor itself. Order: {PRODUCT_NAME}-side question → options → choice. **Pattern-matching failure mode:** reaching for external precedent before stating the {PRODUCT_NAME}-side question. Reverse the order and the work is wrong even if the answer lands right. Sub-agent output where precedent is load-bearing without a {PRODUCT_NAME} terminus gets sent back, not relayed.
...
* **No action is a valid action.** When the operator names a literal target (port, file, path, name, range, id) and zero things match, **do nothing and report empty in one line**. Do not substitute the nearest match. Do not "act anyway" because a tool was invoked or a turn is in progress. The bias toward doing-something instead of doing-the-right-thing is the failure mode — under destructive verbs (kill, rm, reset, stop, drop, force-push) it damages unrelated state. Zero matches → zero action → ask for the right scope. **Incident (2026-06-27):** operator asked to kill Next.js apps on port 3010; nothing existed in that range; killed PM2-managed processes on :3400 and :3600 instead because they were the only Next.js procs visible. Wrong. Empty result was the answer.
...
**Hard stop: before any edit (main thread) or sub-agent dispatch that touches `apps/**`, `packages/**`, or tracked code, read [`.claude/workflows/worktree-protocol.md`](.claude/workflows/worktree-protocol.md) and apply the topic state machine there.** It is the single source of truth for: when to spawn a worktree, solo vs chain coordination, {APP_NAME} port handling (Next.js auto-retries — zero per-worktree config), return + merge protocol, cleanup, and cross-session prune. State the spawn decision in one line before acting. Operator weak-verb authorization ("go", "yes", "ship it") applies to the most recent proposed worktree/branch decision the same way it applies to commit/push. **Design-phase work (docs + `.intermediate/`) runs on `main` — no worktree spawn.**

**Hard stop: before any `gh pr create` or direct merge — code, docs, or workflow — read [`.claude/workflows/pr-self-review.md`](.claude/workflows/pr-self-review.md) and run the loop until clean.** The applicable steps (lint + types, browser verify when the PR touches a UI surface, adversarial re-read always) are non-negotiable; the workflow says which steps apply to which PR shape. The orchestrator owns the loop and runs it *before* dispatching `release-manager` — release-manager does not re-run it. One-pass-and-PR is the failure mode this gate exists to prevent.
